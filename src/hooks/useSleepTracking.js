// useSleepTracking.js with persistent tracking and robust handling
...
const useSleepTracking = () => {
  ...
  const recoverActiveSession = async () => {
    ...
    if (activeSession?.isActive) {
      ...
      if (timeSinceStart < 12 * 60 * 60 * 1000) {
        ...
        // Restore alarm
        const [wakeHours, wakeMinutes] = (activeSession.wakeTime || wakeTime)
          .split(':')
          .map(Number);
  ...
  const startSleepTracking = async () => {
    try {
      ...
      // Request motion permission only inside user gesture
      ...
      if (timeUntilWake > 0) {
        alarmTimeoutRef.current = setTimeout(() => completeSession(sleepSession), timeUntilWake);
      }
      ...
      alert('Failed to start sleep tracking. Check permissions and try again.');
      setIsTracking(false);
      setAlarmSet(false);
      await clearStore(STORE_NAMES.ACTIVE);
    }
